...
This agent uses the **OpenAI Realtime API** for end-to-end voice processing (STT + LLM + TTS in a single model).

- **Voice Model:** OpenAI Realtime (`gpt-realtime-mini`, voice: `cedar`)
- **Voice Activity Detection:** Silero VAD
- **Noise Cancellation:** BVC for regular calls, BVC Telephony for SIP calls
- **Call Classification:** OpenAI GPT-4.1-mini (post-call analysis)

All voice processing runs through the OpenAI Realtime API. Post-call classification uses LiveKit Inference.
...
**Update secrets:**
```bash
lk agent update-secrets --secrets "OPENAI_API_KEY=your-key" --overwrite
```
